...
| File | Behavior |
|---|---|
| `_shared/hustlekit.js` | `TRACKS` catalog (name/icon/craft/offers/hunting/deliverable per track), `MODEL` (`@cf/meta/llama-3.3-70b-instruct-fp8-fast`), `aiJson()` (handles the **pre-parsed `result.response` object** quirk), `normalizeInputs()` (flat checkout params or `{inputs}` wrapper), `trackOf()`, `HONESTY_RULES` (banned phrases (the three prohibited marketing claims — see assignment; the honesty guard blocklists them); no income promises; no unproven superlatives), `json()`, `clientIp()`, `todayDay()`. |
...
5. **Copy**: no banned phrases anywhere (grep-verified during writing — the honesty guard blocklists all three prohibited claims in user-facing copy). No fake testimonials included at all.
